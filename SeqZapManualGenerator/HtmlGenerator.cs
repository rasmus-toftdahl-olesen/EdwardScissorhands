...
namespace SeqZapManualGenerator
{
   ...
   {
      private List<HtmlError> m_errors = new List<HtmlError>();

      public int SmallestLevel { get; set; }
      public string BaseDirectory { get; set; }
      public bool HasErrors { get { return m_errors.Count > 0; } }
      public IList<HtmlError> Errors { get { return m_errors; } }
      ...
      private void AddError( string _text, string _filename )
      {
         HtmlError error = new HtmlError();
         error.Text = _text;
         error.FileName = Path.Combine( BaseDirectory, _filename ) + ".html";
         m_errors.Add( error );
      }
      ...
      private void HtmlTextContent( string _errorFilename, TextContent _content, TextWriter _writer )
      {
         foreach ( TextPart part in _content.Parts )
         {
            switch ( part.Type )
            {
               case TextPartType.Plain:
                  _writer.Write( HtmlEscape( part.Text ) );
                  break;

               case TextPartType.Hyperlink:
                  _writer.Write( "<a href=\"{0}\">{1}</a>", HtmlEscape( part.Url.ToString() ), HtmlEscape( part.Text ) );
                  break;

               case TextPartType.Error:
                  AddError( part.Text, _errorFilename );
                  _writer.Write( "<div class=\"seqzap-manual-generator-error\">{0}</div>", HtmlEscape( part.Text ) );
                  break;
            }
         }
      }
      ...
      public static string HtmlEscape( string _text )
      {
         if ( _text == null )
         {
            return String.Empty;
